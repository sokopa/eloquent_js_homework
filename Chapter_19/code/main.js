...
function drawPicture(picture, canvas, scale, oldPicture = null) {
  if (
    !oldPicture ||
    (picture.width != oldPicture.width && picture.height != oldPicture.height)
  ) {
...
PictureCanvas.prototype.touch = function (startEvent, onDown) {
  ...
  let end = () => {
    this.dom.removeEventListener('touchmove', monve);
    this.dom.removeEventListener('touchend', end);
  };
  thid.dom.addEventListener('touchmove', move);
...
function pictureFromImage(image) {
  ...
  let pixels = [];
  let { data } = cx.getImageData(0, 0, eidth, height);
  function hex(n) {
