...
// Arrays have slice and concat methods, which allow us to easily create new
// arrays without damaging the old one.
...
// given member added and leave the old one unchanged.
...
// definitely want to use it internally). Instead, there is an empty instance, PGroup.empty,
...
class PGroup {
  ...
  add(element) {
    if (!this.has(element)) {
      return new PGroup(this.storage.concat([element]));
    } else return this;
  }

  delete(element) {
    if (this.has(element))
      return new PGroup(this.storage.filter((x) => x !== element));
    else return this;
  }
}
